...
* **Related architecture decisions:** [ADR-001 — Async health check design](architecture/adr/ADR-001.md)
...
* **Related architecture decisions:** [ADR-002 — Scheduler health integration](architecture/adr/ADR-002.md)
...
* **Related architecture decisions:** [ADR-003 — Global exception handling](architecture/adr/ADR-003.md)

---

## MVP v2 Quality Requirements

As `MVP v2` features are implemented, add new quality requirements here following the same structure. Each new requirement must have a stable `QR-NNN` ID, a measurable scenario, linked QRTs, and a related ADR where applicable.
