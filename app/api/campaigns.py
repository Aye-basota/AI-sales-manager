...
@router.post("/{campaign_id}/contacts", response_model=List[CampaignContactResponse], status_code=201)
async def add_contacts_to_campaign(
    ...
    for contact_id in payload.contact_ids:
        cc = CampaignContact(campaign_id=campaign_id, contact_id=contact_id, status="pending", message_count=0)
        db.add(cc)
        created.append(cc)
...
async def stop_campaign(campaign_id: UUID, db: AsyncSession = Depends(get_db)):
    ...
    if campaign.status != "running":
        raise HTTPException(status_code=400, detail="Campaign can only be stopped from running status")
