...
- **LLM-диалоги** — генерация ответов через OpenRouter (Qwen / Gemini / DeepSeek) с guardrails (защита от markdown, признаков бота, повторов).
...
- **LLM:** OpenRouter API (httpx), fallback cascade
...
```bash
cp .env.example .env
# Отредактируй .env — добавь TELEGRAM_API_ID, TELEGRAM_API_HASH, OPENROUTER_API_KEY, ADMIN_BOT_TOKEN
...
**Текущее покрытие:** 377 тестов, покрытие ~77%.
